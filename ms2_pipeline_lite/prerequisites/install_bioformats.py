...
def install_bioformats():
    ...
    loci_tools_path = (
            Path(inspect.getfile(ms2_pipeline_lite)).parents[0] / "loci_tools.jar"
    )

    try:
        shutil.copy(loci_tools_path, pims_path / "loci_tools.jar")
    except OSError:
        warnings.warn("`loci_tools.jar` not found in package folder.")
    ...
    jdk.install("20", path=str(jdk_path))
